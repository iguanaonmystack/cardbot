	...
	def __new__(self, _dealer):
		...
		class cardbot(IRCClient):
			...
			def privmsg(self, user, channel, msg):
				...
				if self.nickname == channel:
					...
					if command == "showhand":
						...
						self.msg(sender, "Unknown command. See " + self.helpURL + " for commands.")
					
				else:
					# otherwise, it's in a channel.
					params = mlc.split(); # words in lower case
					try:
						command = params[1];  # command in lower case
					except IndexError:
						command = ""
					...
					if mlc.startswith(self.nickname.lower()):
						if command == "dealer":
							...
							self.msg(channel, "See " + self.helpURL + " for details and commands.")
					...
					if sender.lower() == self.dealer.lower():
						if command == "reset":
							...
							if len(params) > 3:
								self.dealTo(self.getPlayer(params[2]), int(params[3]), channel)
							else:
								self.dealTo(self.getPlayer(params[2]), 1, channel)
						elif command == "deal":
							if len(params) > 2:
			...
			def initialiseDeck(self, channel):
				self.cards = []
				for deck in [card.CLUBS, card.DIAMONDS, card.HEARTS, card.SPADES]:
			...
			def dealOne(self, p, channel):
				...
				if p != None:
					try:
						c = self.cards.pop(0)
						p.addToHand(c)
						self.msg(p.getName(), "You have been dealt the " + c.toString())
					except Exception, e:
			...
			def dealTo(self, p, numcards, channel):
				try:
					...
					self.msg(channel, str(numcards) + " cards have been dealt to " + p.getName())
				except EmptyDeckError, e:
					self.msg(channel, "The pack has run out of cards. " + str(e.value) + " cards were dealt to " + p.getName() )
			...
			def revealHand(self, name, channel):
				...
				if len(hand) > 0:
					...
					returnstring = returnstring + hand[len(hand) - 1].toString() + "."
				else:
			...
			def pick(self, cardID, sender):
				...
				if c == None:
					...
					if self.getPlayer(sender).pick(c) == True:
						self.msg(sender, "You have picked out the " + c.toString() + ".")
					else:
						self.msg(sender, "You don't have the " + c.toString() + " in your hand.")
			...
			def unpick(self, cardID, sender):
				...
				if c != None:
					if self.getPlayer(sender).unpick(c) == True:
						self.msg(sender, "You have unpicked the " + c.toString() + ".")
					else:
						self.msg(sender, "You don't have the " + c.toString() + " currently picked.")
				else:
					self.msg(sender, "That's not a valid card selection.")
			...
			def giveback(self, name, channel):
				...
				for i in returnedcards:
					self.cards.append(i)
				self.msg(channel, name + " returned " + str(len(returnedcards)) + " cards to the bottom of the deck.")
			
			def givehandto(self, fromwhom, to, channel):
				p = self.getPlayer(fromwhom)
				if self.getPlayer(to) != None:
					self.msg(channel, fromwhom + " tried to transfer their cards to " + to + ", but " + to + " is already a player")
				else:
					p.setName(to)
					self.msg(channel, fromwhom + " has transferred their cards to " + to)
			...
			def offer(self, offerer, player, numcards, channel):
				...
				if numcards != 0:
					self.msg(channel, offerer + " has offered " + numcards + " cards to " + player)
				else:
					self.msg(channel, offerer + " has withdrawn their offer of cards")
...
class card:
	...
	def toString(self):
		return self.getValueAsString(self.value) + " of " + self.getSuitAsString(self.suit)
		
	def __cmp__(self, other):
		if other != None: 
			return cmp(self.getValue() + self.getSuit(), other.getValue() + other.getSuit())
		return 1
